...
import com.example.pexelsapp.domain.CollectionRepository
...
class HomeScreenViewModel @Inject constructor(
    ...
    private fun chooseCollection(text: String) {
        ...
        currentListCollections.forEach { collection ->
            if (
                ...
                newCurrentListCollections.add(CollectionItem(collection.title, true))
            } else {
                newCurrentListCollections.add(collection.copy(isSelected = false))
            }
        }
    ...
    private fun searchPhotos(text: String) {
        ...
        viewModelScope.launch {
            try {
                val listPhotos = photoRepository.getSearchPhotos(text)
    ...
    private fun convertCollectionsItems(collection: List<Collection>): List<CollectionItem> {
        ...
        collection.forEach {
            collectionsItemsList.add(CollectionItem(it.title, false))
        }
    ...
    private fun loadPhotos() {
        ...
        viewModelScope.launch {
            try {
                val collections = collectionRepository.getFeaturedCollections()
                firstCollection = collections[0].title
                _state.value =
                    HomeScreenState(
                        photoRepository.getPhotos(),
                        convertCollectionsItems(collections)
                    )
                val currentState = _state.value ?: return@launch
                _uiState.value = UiState.Loaded(currentState)
            } catch (e: UnknownHostException) {
